...
public class HeroSpell : MonoBehaviour
{
	...
	void OnMouseOver()
	{
		...
		if (controller.infoBar == null)
		{
			...
			Info.text = Information.GetSpellInfo(spell.Name, controller.friends[num].SpellLevel[num], controller.friends[num].power);
			foreach (var x in spell.statusName)
			{
				Info.text += '\n' + Information.GetEffectInfo(x, controller.friends[num].SpellLevel[num], controller.friends[num].power);
			}
